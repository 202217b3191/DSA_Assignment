...
void initializeHashMap(struct hashMap* mp) {
  ...
  for (int i = 0; i < mp->capacity; ++i) {
    mp->arr[i] = NULL;  // Initialize all elements to NULL
  }
  return;
}
...
void insert(struct hashMap* mp, const char* key, const char* value) {
  ...
  // Collision
    ...
    // Adding newNode at the head of linked list which is present
    // at bucket index....insertion at head in linked list
